...
(function ( $ ) {
    ...
    $.fn.treejs = function( options ) {
        ...
        else
        {
            ...
            for(var i in data)
            {
                ...
                {
                    html += '<ul class="jtree_parent jtree_parent_node"><li class="jtree_node"><span class="jtree_expand jtree_node_open"> </span> <input type="checkbox" id="'+ data[i]['parentNodeId'] +'" parent-id="" class="form-check-input jtree_parent_checkbox"> <label class="form-check-label">'+ data[i]['parentNodeTxt'] +'</label>';
                    html += '<ul class="jtree_parent jtree_child_node">';

                    for(var j in data[i]['childNodes'])
                    {
                        if( data[i]['childNodes'].hasOwnProperty(j) )
                        {
                            html += '<li class="jtree_node"><input type="checkbox" id="'+ data[i]['childNodes'][j]['id'] +'" parent-id="'+ data[i]['parentNodeId'] +'" class="form-check-input jtree_child_checkbox"> <label class="form-check-label">'+ data[i]['childNodes'][j]['name'] +' </label></li>';
                        }
                    }
		...
		$('.jtree_expand').on('click', function(){
		...
		$('.jtree_parent_checkbox').on('change', function(){
			
			if( $(this).is(':checked') )
			{
				var childUL = $(this).parent().children().next('ul');
    ...
    $.fn.extend({
        ...
        getCheckedChildNodes: function() {
            return $(this).find('.form-check-input:checked');
        },
        getUncheckedChildNodes: function() {
            return $(this).find('.form-check-input:not(:checked)');
        }
    });
